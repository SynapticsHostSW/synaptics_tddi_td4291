...
#define DSI_DT_DCS_GENERIC_READ_0	0x04
#define DSI_DT_DCS_GENERIC_READ_1	0x14
#define DSI_DT_DCS_GENERIC_READ_2	0x24
...
#define DSI_DT_RX_GENERIC_READ_1	0x11
#define DSI_DT_RX_GENERIC_READ_2	0x12
	...
	while (REG_GET(dsidev, DSI_VC_CTRL(channel), 20, 20)) {
		...
		if (dt == DSI_DT_RX_ACK_WITH_ERR) {
			...
			dsi_show_rx_ack_with_err(err);
				...
				(dt == DSI_DT_RX_GENERIC_READ_1)) {
			DSSERR("\tDCS short response, 1 byte: %#x\n",
					FLD_GET(val, 23, 8));
		} else if ((dt == DSI_DT_RX_SHORT_READ_2) ||
				(dt == DSI_DT_RX_GENERIC_READ_2)) {
...
int dsi_vc_dcs_generic_write_nosync(struct omap_dss_device *dssdev, int channel,
		u8 *data, int len)
{
	struct platform_device *dsidev = dsi_get_dsidev_from_dssdev(dssdev);
	int r;
...
EXPORT_SYMBOL(dsi_vc_dcs_generic_write_nosync);
...
int dsi_vc_dcs_generic_write(struct omap_dss_device *dssdev, int channel,
		u8 *data, int len)
{
	struct platform_device *dsidev = dsi_get_dsidev_from_dssdev(dssdev);
	int r;

	r = dsi_vc_dcs_generic_write_nosync(dssdev, channel, data, len);
	if (r)
		goto err;
	...
	return 0;
err:
	DSSERR("dsi_vc_dcs_generic_write(ch %d, cmd 0x%02x, len %d) failed\n",
			channel, data[0], len);
	return r;
}
EXPORT_SYMBOL(dsi_vc_dcs_generic_write);
...
int dsi_vc_dcs_generic_read(struct omap_dss_device *dssdev, int channel,
		u8 dcs_cmd, u8 *buf, int buflen)
{
	...
	if (dsi->debug_read)
		DSSDBG("dsi_vc_dcs_generic_read(ch%d, dcs_cmd %x)\n", channel,
				dcs_cmd);

	if (buflen == 1)
		r = dsi_vc_send_short(dsidev, channel,
				DSI_DT_DCS_GENERIC_READ_1,
				dcs_cmd, 0);
	else if (buflen == 2)
		r = dsi_vc_send_short(dsidev, channel,
				DSI_DT_DCS_GENERIC_READ_2,
				dcs_cmd, 0);
	else
		r = dsi_vc_send_short(dsidev, channel,
				DSI_DT_DCS_READ,
	...
	if (dt == DSI_DT_RX_ACK_WITH_ERR) {
	...
	} else if (dt == DSI_DT_RX_GENERIC_READ_1) {
		u8 data = FLD_GET(val, 15, 8);
		if (dsi->debug_read)
			DSSDBG("\tDCS short response, 1 byte: %02x\n", data);
		...
		return 1;
	} else if (dt == DSI_DT_RX_GENERIC_READ_2) {
		u16 data = FLD_GET(val, 23, 8);
		if (dsi->debug_read)
			DSSDBG("\tDCS short response, 2 byte: %04x\n", data);
	...
	BUG();
err:
	DSSERR("dsi_vc_dcs_generic_read(ch %d, cmd 0x%02x) failed\n", channel,
			dcs_cmd);
	return r;

}
EXPORT_SYMBOL(dsi_vc_dcs_generic_read);
