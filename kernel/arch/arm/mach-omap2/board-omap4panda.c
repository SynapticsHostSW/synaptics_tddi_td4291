...
#define TM2448 (2)
#define TD4291 (3)
#define SYNAPTICS_MODULE TD4291
...
#if (SYNAPTICS_MODULE == TD4291)
#define DSX_I2C_ADDR 0x2c
#define DSX_ATTN_GPIO 52
#define DSX_ATTN_MUX_NAME "gpmc_ncs2.gpio_52"
#define DSX_RESET_GPIO -1
#define DSX_SWITCH_GPIO 121
#define DSX_SWITCH_MUX_NAME "abe_dmic_din2.gpio_121"

static unsigned char td4291_cap_button_codes[] = {};

static struct synaptics_dsx_cap_button_map td4291_cap_button_map = {
	.nbuttons = ARRAY_SIZE(td4291_cap_button_codes),
	.map = td4291_cap_button_codes,
};
...
static struct synaptics_dsx_platform_data dsx_platformdata = {
	...
	.switch_gpio = DSX_SWITCH_GPIO,
 	.gpio_config = synaptics_gpio_setup,
 	.cap_button_map = &td4291_cap_button_map,
};
...
#endif
#ifdef DSX_SWITCH_MUX_NAME
		omap_mux_init_signal(DSX_SWITCH_MUX_NAME, OMAP_PIN_INPUT_PULLUP);
#endif
		omap_register_i2c_bus(2, 400, bus2_i2c_devices, ARRAY_SIZE(bus2_i2c_devices));
	} else {
...
static int dsi1_panel_set_backlight(int level)
{
	int r;

	r = twl_i2c_write_u8(TWL_MODULE_PWM, 0x7F, LED_PWM1OFF);
	if (r)
		return r;
	...
	if (level > 1) {
		...
		r = twl_i2c_write_u8(TWL_MODULE_PWM, level, LED_PWM1ON);
		if (r)
			return r;

		r = twl_i2c_write_u8(TWL6030_MODULE_ID1, 0x06, TWL6030_TOGGLE3);
		if (r)
			return r;
	} else if (level <= 1) {
		r = twl_i2c_write_u8(TWL6030_MODULE_ID1, 0x01, TWL6030_TOGGLE3);
		if (r)
			return r;

		r = twl_i2c_write_u8(TWL6030_MODULE_ID1, 0x05, TWL6030_TOGGLE3);
		if (r)
			return r;
...
static struct panel_td4291_data dsi1_panel =
{
	.name = "td4291",
	.debug_mode_gpio = 101,
	.reset_gpio = 102,
	.vid_if_sel0_gpio = 155,
	.vid_if_sel1_gpio = 156,
...
static void omap4_panda_dsi_init(void)
{
	u32 reg;
	int status;
...
/*
	...
	status = gpio_request_one(dsi1_panel.vid_if_sel0_gpio, GPIOF_DIR_OUT,
		"dsi_vid_if_sel0_gpio");
	if (status)
		pr_err("%s: Could not get vid if sel0 gpio\n", __func__);
	else
		gpio_set_value(dsi1_panel.vid_if_sel0_gpio, 0);

	omap_mux_init_signal("uart4_tx.gpio_156", OMAP_PIN_OUTPUT);
	status = gpio_request_one(dsi1_panel.vid_if_sel1_gpio, GPIOF_DIR_OUT,
		"dsi_vid_if_sel1_gpio");
	if (status)
		pr_err("%s: Could not get vid if sel1 gpio\n", __func__);
	else
		gpio_set_value(dsi1_panel.vid_if_sel1_gpio, 1);
*/
	return;
}
...
#if MIPI_DSI
	.default_device = &omap4_panda_dsi_device,
#else
